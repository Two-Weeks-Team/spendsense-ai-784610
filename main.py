...
def on_startup() -> None:
    ...
    try:
        ...
        if not db.query(User).filter(User.id == demo_id).first():
            db.add(
                User(
                    id=demo_id,
...
<div class="max-w-3xl mx-auto px-4 py-10 sm:py-16">
  ...
  <section class="bg-[#171717] border border-[#262626] rounded-xl p-5 sm:p-6 mb-4">
    <h2 class="text-lg font-bold text-[#f5f5f5] mb-3">How It Works</h2>
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
      <div class="text-center"><div class="text-2xl mb-1">📄</div><p class="text-sm font-medium text-[#e5e5e5]">1. Upload CSV</p><p class="text-xs text-[#737373]">Upload your bank transaction CSV file.</p></div>
      <div class="text-center"><div class="text-2xl mb-1">🤖</div><p class="text-sm font-medium text-[#e5e5e5]">2. AI Categorizes</p><p class="text-xs text-[#737373]">AI automatically categorizes each expense.</p></div>
      <div class="text-center"><div class="text-2xl mb-1">💡</div><p class="text-sm font-medium text-[#e5e5e5]">3. Savings Plan</p><p class="text-xs text-[#737373]">Get personalized tips to save money.</p></div>
      <div class="text-center"><div class="text-2xl mb-1">📊</div><p class="text-sm font-medium text-[#e5e5e5]">4. Weekly Report</p><p class="text-xs text-[#737373]">See your spending breakdown at a glance.</p></div>
    </div>
  </section>
  ...
  <footer class="text-center mt-8 text-xs text-[#525252]">
    ...
    &bull; <a href="/docs" class="text-[#525252] hover:text-[#737373]">API Docs</a>
  </footer>
...
document.getElementById('uploadForm').addEventListener('submit', async function(e) {
  ...
  try {
    ...
    _lastUploadId = data.upload_id || data.id || null;
...
document.getElementById('categorizeForm').addEventListener('submit', async function(e) {
  ...
  try {
    if (!_lastUploadId) { throw new Error('Please upload a CSV file first'); }
    const body = { upload_id: _lastUploadId, model_version: document.getElementById('modelVersion').value || null };
