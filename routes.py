...
async def upload_csv(
    ...
    if not required.issubset(set(header)):
        raise HTTPException(
            status_code=400, detail=f"CSV missing columns: {required - set(header)}"
        )
    for row in lines[1:]:
        cols = row.split(",")
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Error parsing row: {row}. {exc}"
            )
    db.commit()
...
async def categorize_transactions(
    ...
    model_version = payload.model_version or db.scalar(
    ...
    return CategorizeResponse(
        transactions=[TransactionOut(**c) for c in categorized],
...
async def generate_savings_plan(
    ...
    if not txns:
        raise HTTPException(
            status_code=404, detail="No transactions in the given period"
        )
    tx_data = [
        {
        ...
        recommendations=[
            SavingsPlanItem(
                ...
                estimated_monthly_savings=float(
                    item.get("estimated_monthly_savings", 0.0)
                ),
            )
